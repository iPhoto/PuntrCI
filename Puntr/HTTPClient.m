...
{
    [self JSONWithRequestMethod:RKRequestMethodPOST
                           path:APIAuthorization
                     parameters:@{ KeyLogin: login, KeyPassword: password }
...
{
    [self JSONWithRequestMethod:RKRequestMethodDELETE
                           path:APIAuthorization
                     parameters:@{ KeySID: self.sid }
                        success:success
                        failure:failure];
}
                  ...
                  failure:(JSONFailure)failure
{
    [self JSONWithRequestMethod:RKRequestMethodPOST
                           path:APIUsers
                     parameters:@{ KeyEmail: email, KeyPassword: password, KeyFirstName: firstname, KeyLastName: lastname, KeyUsername: username }
                        success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON)
                        {
                            if (success)
                            {
...
- (void)JSONWithRequestMethod:(RKRequestMethod)requestMethod
                         path:(NSString *)path
