...
@interface TNIEventCell ()

@property (nonatomic, strong) TNIEvent *event;
...
- (void)loadWithEvent:(TNIEvent *)event {
    ...
    for (TNIParticipants *participant in event.participants) {
        if (counter == 0) {
...
- (void)stakeButtonTouched {
    [[TNIObjectManager sharedManager] setStakeForEvent:self.event.tag success:nil failure:nil];
}
